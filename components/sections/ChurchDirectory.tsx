...
const nationalRegions: Region[] = [
  {
    ...
    churches: [
      { name: "JLYCC Dasmariñas", pastor: "Pastor Vivian O. Adarle", address: "B106 L008 Brgy. Sta. Lucia, Dasmariñas City, Cavite", fb: "https://www.facebook.com/jlycc.dasmarinas.9" },
...
function ChurchCard({ church, regionName }: { church: Church; regionName: string }) {
  return (
    <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
      <div className="flex items-start justify-between gap-2 mb-1">
        <div className="flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-teal-500 mt-1 shrink-0" />
          <span className="font-bold text-gray-900 text-sm leading-snug">{church.flag && `${church.flag} `}{church.name}</span>
        </div>
      ...
      {church.pastor && (
        <p className="text-teal-500 text-xs font-medium mb-1 pl-4 italic">{church.pastor}</p>
      )}
      {church.address && (
        <a
          ...
          className="text-blue-500 hover:text-blue-700 text-xs pl-4 leading-snug block mb-2 underline underline-offset-2"
        >
          {church.address}
        </a>
      )}
      {church.fb && (
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="text-teal-500 hover:text-teal-700 text-xs font-semibold pl-4"
        >
          FB Page
        </a>
      )}
...
function RegionAccordion({ region, isOpen, onToggle, color }: { region: Region; isOpen: boolean; onToggle: () => void; color: "teal" | "crimson" }) {
  ...
  return (
    <div className={`rounded-xl border overflow-hidden transition-all duration-200 ${isOpen ? (teal ? "border-teal-500 bg-teal-600" : "border-teal-500 bg-teal-600") : "border-teal-400/60 bg-teal-500"}`}>
      <button
        onClick={onToggle}
                ...
                <div key={church.name} className={`px-4 py-3 ${i !== region.churches.length - 1 ? "border-b border-gray-100" : ""}`}>
                  ...
                  {church.address && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-blue-500 hover:text-blue-700 text-xs mt-0.5 pl-4 leading-snug block underline underline-offset-1"
                    >
                      {church.address}
                    </a>
                  )}
                  {church.fb && (
                    <a href={church.fb} target="_blank" rel="noopener noreferrer" className="text-teal-500 hover:text-teal-700 text-xs font-semibold pl-4 mt-1 block">
                      FB Page
                    </a>
                  )}
                </div>
    ...
    >
      <div className="container-section" ref={ref}>
        ...
        >
          ...
          <h2 id="directory-heading" className="font-heading text-4xl md:text-5xl font-black text-gray-900 mt-2">
            CHURCH DIRECTORY
          </h2>
        ...
        >
          <div className="flex gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full bg-white border border-gray-200 rounded-2xl pl-12 pr-10 py-4 text-gray-800 placeholder-gray-400 text-sm focus:outline-none focus:border-teal-500 transition-colors shadow-sm"
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-2 bg-teal-500 hover:bg-teal-600 text-white font-bold text-sm px-5 py-4 rounded-2xl transition-colors whitespace-nowrap shadow-sm"
            >
              <Navigation size={16} />
              Near Me
            </a>
          ...
          {search && (
            <p className="text-sm text-gray-500 mt-3 pl-1">
              Found <span className="font-bold text-teal-600">{searchResults.length}</span> location{searchResults.length !== 1 ? "s" : ""}
            </p>
          )}
        </motion.div>
            ...
            {searchResults.length > 0 ? (
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {searchResults.map(({ church, regionName }) => (
                  <ChurchCard key={`${regionName}-${church.name}`} church={church} regionName={regionName} />
                ))}
              </div>
            ) : (
          ...
          <>
            ...
            >
              <div className="flex items-center gap-3 mb-5">
                ...
                <h3 className="font-heading font-black text-gray-800 text-lg">International Churches</h3>
              </div>
            ...
            >
              <div className="flex items-center gap-3 mb-5">
                ...
                <h3 className="font-heading font-black text-gray-800 text-lg">
            ...
            >
              <a href="/find-a-church" className="inline-flex items-center gap-2 bg-teal-500 hover:bg-teal-600 text-white font-bold px-8 py-4 rounded-2xl transition-colors">
                <MapPin size={16} />
